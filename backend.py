...
def clean_price(price_str):
    ...
    try:
        # Fiyat temizleme: Noktaları sil, virgülü noktaya çevir
...
def compare():
    ...
    for item in results:
        ...
        # Sadece senin fiyatından DAHA UCUZ olanlar
        if current_page_price > 0 and price_val >= current_page_price:
            continue
        
        # Link yoksa veya hatalıysa ekleme
        if not item_link or not str(item_link).startswith("http"):
            continue

        if any(w in item.get("source", "").lower() for w in whitelist) or ".tr" in item_link.lower():
